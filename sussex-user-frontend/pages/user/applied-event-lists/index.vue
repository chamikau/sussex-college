      ...
      <div class="page-header d-print-none">
        ...
        <div class="row g-2 align-items-center">
          <div  class="col d-flex align-items-center">
            <span class="fw-bold fs-1">Applied Events</span>
          </div>
          <div class="card">
              ...
              <div class="contents">
                <Table :attributes="attributes" @inputUpdates="inputUpdates">
                  ...
                  <template v-slot:event_type_name="props">
                    {{props.record.events.event_type.name}}
                  </template>
                  <template v-slot:date="props">
                    {{props.record.events.date}}
                  </template>
                  <template v-slot:location="props">
                    {{firstLetterUpperCase(props.record.events.location)}}
                  </template>
                  <template v-slot:location="props">
                    {{firstLetterUpperCase(props.record.events.location)}}
                  </template>
...
export default {
  ...
  methods: {
    async goToViewDetails(props) {
      await this.$router.push(`/user/applied-event-lists/${props.record.events.id}/application`)
    },
    async getEvents(page = 1, sort = '', search = '', per_page = 15,) {
      try {
        this.attributes.api_response = await this.$axios.$get('/api/student/student-applied-events',
          {
            params: {
              per_page: per_page,
              page: page,
    ...
    selectEvent() {
      this.attributes.filters['filter[events.event_type_id]'] = this.eventAttributes.selected.value;
      this.filterUpdates()
    },
    inputUpdates(values) {

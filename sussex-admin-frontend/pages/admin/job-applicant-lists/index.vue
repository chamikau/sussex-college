      ...
      <div class="page-header d-print-none">
        ...
        <div class="row g-2 align-items-center">
          <div class="col d-flex align-items-center">
            <span class="fw-bold fs-1">Job Applicants</span>
          </div>
          <div class="card">
            <div class="card-body"><h3 class="card-title">Filters</h3>
              <div class="row test-1">
                <div class="col-md-4">
                  ...
                  <input
                    v-model="attributes.filters['filter[jobs.title]']"
                    @keyup="filterUpdates"
                    type="text"
                    class="form-control"
              ...
              <div class="contents">
                <Table :attributes="attributes" @inputUpdates="inputUpdates">
                  <template v-slot:title="props">
                    {{props.record.jobs.title}}
                  </template>
                  <template v-slot:location_type="props">
                    {{firstLetterUpperCase(props.record.jobs.location_type)}}
                  </template>
...
export default {
    ...
    return {
      ...
      attributes: {
        ...
        labels: [
          ...
          {key: 'location_type', name: 'Location Type', sort: true},
  ...
  methods: {
    ...
    //   await this.$router.push(`/admin/job-applicant-lists/${props.record.student_id}/application`)
    // },
    async goToViewDetails(props) {
      const studentId = props.record.students.id;
      const jobId = props.record.jobs.id;
      await this.$router.push({
        path: `/admin/job-applicant-lists/job-application-view/${studentId}/general`,
        query: { studentId ,jobId },
      });
    },
    async getJobApplicantLists(page = 1, sort = '', search = '', per_page = 15,) {
      try {
        this.attributes.api_response = await this.$axios.$get('/api/admin/student-applied-lists',
          {
            params: {
              per_page: per_page,
              page: page,
    ...
    inputUpdates(values) {
      ...
      this.getJobApplicantLists(values.page, values.sort.sort_input, values.search, values.per_page)
    },
    filterUpdates() {
      const values = this.attributes.table_component_values
      this.getJobApplicantLists(1, values.sort.sort_input, values.search, values.per_page)
    },
    filterClear() {
      this.attributes.filters = {}
      this.getJobApplicantLists()
    }
  },

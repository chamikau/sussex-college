      ...
      <div class="page-header d-print-none">
        ...
        <div class="row g-2 align-items-center">
          <div class="col d-flex align-items-center">
            <span class="fw-bold fs-1">Jobs List</span>
          </div>
          <div class="card">
...
export default {
    ...
    return {
      ...
      attributes: {
        ...
        labels: [
          ...
          {key: 'number_of_openings', name: 'Number Of Openings', sort: true},
          {key: 'location_type', name: 'Location', sort: true},
          {key: 'probation_duration', name: 'Probation Duration', sort: true},
  ...
  methods: {
    async goToViewDetails(props) {
      ...
      await this.$router.push(`/admin/jobs/${props.record.id}/application`)
    },
    // async activeOrInactiveJob(job) {
    //   try {
    //     if (job.state === 'inactive') {
    //       await this.$axios.$post(`/api/admin/jobs/${job.id}/active`)
    //     } else if (job.state === 'active') {
    //       await this.$axios.$post(`/api/admin/jobs/${job.id}/inactive`)
    //     }
    //     await this.getJobs();
    //   } catch (e) {
    //     return e;
    ...
    async getJobs(page = 1, sort = '', search = '', per_page = 15,) {
      try {
        this.attributes.api_response = await this.$axios.$get('/api/admin/jobs',
          {
            params: {
              per_page: per_page,
              page: page,
